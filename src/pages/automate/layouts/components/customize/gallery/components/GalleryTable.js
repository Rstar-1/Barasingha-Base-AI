...
import AddGallery from "./add/AddGallery";
...
const GalleryTable = () => {
  ...
  const getdata = useSelector(
    (state) => state.gallerydata.gallerydata.gallerystore
  );
  const totalCount = useSelector((state) => state.gallerydata.totalCount);
  // Redux State
  ...
  const handleDelete = async (id) => {
    try {
      const resultAction = await dispatch(deletegallery(id));
  ...
  const handlePublish = async (id, newstatus) => {
    try {
      ...
      await dispatch(statusgallery({ id, data }));
      dispatch(paginationgallery({ offset: currentpage * 6, search }));
