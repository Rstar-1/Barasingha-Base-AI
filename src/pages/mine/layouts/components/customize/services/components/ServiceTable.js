...
const ServiceTable = () => {
  ...
  const getdata = useSelector(
    (state) => state.servicedata.servicedata.servicestore
  );
  const totalCount = useSelector((state) => state.servicedata.totalCount);
  // Redux State
  ...
  const handlePublish = async (id, newstatus) => {
    try {
      ...
      await dispatch(statusservice({ id, data }));
      dispatch(paginationservice({ offset: currentpage * 6, search }));
