...
const TeamTable = () => {
  ...
  const getdata = useSelector((state) => state.teamdata.teamdata.teamstore);
  const totalCount = useSelector((state) => state.teamdata.totalCount);
  // Redux State
  ...
  const handleDelete = async (id) => {
    try {
      const resultAction = await dispatch(deleteteam(id));
  ...
  const handlePublish = async (id, newstatus) => {
    try {
      ...
      await dispatch(statusteam({ id, data }));
      dispatch(paginationteam({ offset: currentpage * 6, search }));
